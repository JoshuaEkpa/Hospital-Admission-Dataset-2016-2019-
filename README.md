# Hospital-Admission-Dataset-2016-2019-

<img width="1600" height="990" alt="image" src="https://github.com/user-attachments/assets/b69c9f83-dca2-4e06-bbec-01ddd7e62cd0" />








___________________________________________________________________________________________________________________________________________________ 

This study applies Exploratory Data Analysis (EDA) to hospital admission records covering the period 2016–2019, with the objective of identifying statistically meaningful patterns in patient demographics, disease prevalence, and healthcare utilization. Using secondary administrative health data, key variables


___________________________________________________________________________________________________________________________

🎯 Business Objectives

The key objectives of this project are to:
1. Analyze hospital admission trends over time
2. Examine patient distribution by age group and gender
3. Identify the most common diagnoses and medical conditions
4. Evaluate average length of hospital stay
6. Provide actionable insights for healthcare planning and resource allocation

_____________________________________________________________________________________________________________________________

📊 Dataset 

1. Column Description
2. Column Name       ------>                        Description
3. SN                ------>                        Unique patient record ID
4. Age               ------>                        Patient age
5. Age_Group         ------>                        Age range category
6. Gender            ------>                        M = Male, F = Female
7. Diagnosis         ------>                        Primary medical condition
8. DOA               ------>                        Date of Admission
9. Length_of_Stay    ------>                        Number of days admitted
10. Outcome          ------>                        Discharged / Referred / Deceased
11. Type_of_Area     ------>                        U = Urban, R = Rural

_____________________________________________________________________________________________________________________________


🛠 Tools & Technologies
1. Microsoft Excel – Data cleaning and preprocessing
2. Power BI – Data modeling, visualization, and dashboard creation

______________________________________________________________________________________________________________________

📊 Key Metrics 
1. Total Admission: 15,757 Patients
2. Average Length of Stay: 6.42days
3. Time Period Covered: 2016 - 2019

___________________________________________________________________________________________________________________________ 

📊 Keys Insights 
1. Age Age: Patients aged 60 - 73 years recoreded the highest number of admissions, indicating increased healthcare demand among the elderly
2. Gender Distribution: Male patients accounted for approximately 63% of the total admissions, while femmales represnted 37%.
3. Top Diagnoses: Cardiovascular and Metabolic conditions dominate admissions, including:
   i. Coronary Artery Disease (CAD)
   ii. HYpertension (HTN)
   iii. Acute Coronary Syndrome (ACS)
   iv. Diabetes Mellitus (DM)
4. Area Observation: Admission records peak in 2016, with lower values in subsequent years, suggesting possible data aggregation or reporting limitation  

______________________________________________________________________________________________________________________________ 

📈 Dasboard Features 
1. KPI cards for total admissions and average lengt of stay
2. Age group distribution analysis
3. Gender-based outcome visualization using treemap
4. Admission trends by date of admission (DOA)
5. Urban vs Rural admission comaprison 

_______________________________________________________________________________________________________________________________ 

📊 Conclusion 

The forecast indicates a steady increase in hospital admissions, largely driven by aging populations and chronic disease prevalence. Proactive planning in staffing, infrastructure, and preventive healthcare will be essential to meet future demand.

______________________________________________________________________________________________________________________________

📈 Recommendations

1. Increase geriatric-focused healthcare services
2. Strengthen cardiovascular disease prevention and management programs
3. Investigate data gaps in admission records across years
4. Optimize hospital bed management to reduce length of stay 

_________________________________________________________________________________________________________________

Dashboard Preview 
<img width="1182" height="670" alt="EDA HOSPITAL ADMISSION DATASET Screenshot 2025-11-21 175858" src="https://github.com/user-attachments/assets/60ed0d63-7b4b-4b55-9efc-e9f9d91dd0b4" />


__________________________________________________________________________________________________________________________________ 

Age-Group Diagnoses 
<img width="1189" height="668" alt="Age-Group Diagnoses Screenshot 2025-12-20 175718" src="https://github.com/user-attachments/assets/623ebf88-c7af-4adb-a40a-1ed9372e5334" />

_________________________________________________________________________________________________________________________________ 

Let's Dive Into The World Of DATA
<img width="586" height="400" alt="Screenshot 2025-12-20 192650" src="https://github.com/user-attachments/assets/71bef6ae-55b3-4593-9d0a-8e3fb29008c5" />





